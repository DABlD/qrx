...
use App\Models\{Device, Ad, AuditTrail};
use DB;
...
class AdController extends Controller
{
    ...
    public function store(Request $req){
        ...
        $data->save();
        $this->log(auth()->user()->fullname, 'Create Ad', "Ad ID: " . $data->id);
    }

    public function update(Request $req){
        $update = DB::table($this->table)->where('id', $req->id)->update($req->except(['id', '_token']));
        $this->log(auth()->user()->fullname, 'Updated Ad', "ID: $req->id");
    }

    public function delete(Request $req){
        Ad::find($req->id)->delete();
        $this->log(auth()->user()->fullname, 'Delete Ad', "ID: $req->id");
    }
